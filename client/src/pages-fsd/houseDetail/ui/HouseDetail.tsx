...
import {Avatar, Button, Dialog, Modal, Text} from "@gravity-ui/uikit";
...
const HouseDetail = () => {
    ...
    useEffect(() => {
        const storedHouses: House[] | null = localStorageService.getItem("houses");
        setHouses(storedHouses);

        const data = storedHouses?.find((item) => item.id === params?.id);
        setisFill(!!data);
    }, [params?.id]);
    ...
    return (
        <div>
            <div className={styles.header}>
                ...
                <DialogGallery images={data?.house.images} open={openDialog} setOpen={setOpenDialog}/>
                <DialogAppeal
            ...
            <div className={styles.gallery} onClick={() => setOpenDialog(true)}>
                <div>
            ...
            <div className={styles.main}>
                <div className={styles.main_content}>
                    ...
                    {data?.house.lng && <HouseMap lng={data?.house.lng} lat={data?.house.lat}/>}
...
const HouseMap = ({lat, lng}: MapPropsType) => {
    const defaultState = {
    ...
    return (
        <>
            <Text variant="header-1">Карта</Text>
...
type PropsDialogGalleryType = {
    open: boolean;
    setOpen: (open: boolean) => void;
    images?: File[] | null;
}
const DialogGallery = ({open, setOpen, images}: PropsDialogGalleryType) => {
    ...
    return (
        ...
        >
            <Dialog.Body>
                <Text variant="display-1">Галерея</Text>
                ...
                {images?.length ? (
                    ...
                    >
                        {images.map((file) => (
                            <SwiperSlide key={file.id} style={{height: "80vh"}}>
                                <Image
                                    src={file.path}
                                    alt={file.name ?? ''}
                                    fill
                    ...
                    <Text variant="body-short" style={{marginTop: 16}}>Нет изображений</Text>
                )}
            </Dialog.Body>
        </Dialog>
    );
